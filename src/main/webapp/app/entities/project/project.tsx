    ...
    return (
      <div>
        <h2 id="project-heading">
          <Translate contentKey="brazilbmteApp.project.home.title">Projects</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />&nbsp;
            <Translate contentKey="brazilbmteApp.project.home.createLabel">Create new Project</Translate>
          </Link>
        </h2>
            ...
            <tbody>
              {projectList.map((project, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{project.towerCount}</td>
                  <td>{project.geometryJson ? <Link to={`geometry/${project.geometryJson.id}`}>{project.geometryJson.id}</Link> : ''}</td>
